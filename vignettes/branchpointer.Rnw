...
Query files can be read into branchpointer using readQueryFile(), and location information reteived for these sequences:
...
queryIntronFromGTF <- makeBranchpointWindowForExons("ENSE00003541068.1", 
                                                    idType = "exon_id", 
                                                    exons = exons)
head(queryIntronFromGTF)

# for multiple ids:
queryIntronFromGTF <- makeBranchpointWindowForExons(c("ENSE00003541068.1", "ENSE00003461148.1"), 
                                                    idType = "exon_id", 
                                                    exons = exons)
head(queryIntronFromGTF)
@
...
When reading in large numbers of SNPs, it is reccomended to set filter = TRUE.
...
querySNP <- makeBranchpointWindowForSNP(c("rs17000647","rs998731"), 
                                        mart.snp = mart, 
                                        exons = exons, 
                                        filter  = FALSE)
head(querySNP)
